-- שלב 31 (חלק ב'): יבוא תלמידים מורחב - יצירה + שיוך לקבוצה + חשבון בנק בהעלאה אחת.
--
-- עד כה היבוא יצר תלמידים בלבד, וקובץ אמיתי של 1583 תלמידים היה מייצר 1583
-- טיוטות בלי שיוך ובלי חשבון - עבודת המשך שאי אפשר לעשות ידנית בהיקף כזה.
-- הקובץ ממילא מכיל את כל המידע (שם קבוצה, סמל מוסד, מספר סניף, ובנק/סניף/חשבון).
--
-- DROP לפני CREATE ולא רק CREATE OR REPLACE: נוספו שני ערכי החזרה, ו-PostgreSQL
-- אינו מרשה שינוי טיפוס ההחזרה של פונקציה קיימת דרך replace.
--
-- דורש את מיגרציה 092 (טבלת הבנקים ו-resolve_bank_code).

drop function if exists commit_students_import_batch(uuid);
