-- שלב 32 (השלמה): קוד בנק גם לחשבון של העמותה.
--
-- מיגרציה 092 הוסיפה bank_code ל-student_bank_accounts בלבד, ופספסה את
-- organization_bank_accounts. הפער התגלה כשהמסך עבר לבחירת בנק מרשימה: הבורר
-- מחזיר קוד, ולא היה לאן לשמור אותו.
--
-- למה זה נחוץ ולא רק "נחמד": קובץ מס"ב מזהה בנק במספר ולא בשם. חשבון של עמותה
-- הוא הצד *ששולח* את הכסף, ולכן בלי הקוד לא ניתן לייצר ממנו קובץ תשלום - וזה
-- היה מתגלה רק בתשלום הראשון.

alter table organization_bank_accounts add column if not exists bank_code text;

-- ה-view מוסיפה עמודה בסופה. ‎create or replace view‎ מרשה הוספה בסוף בלבד, ולכן
-- שאר העמודות נשארות בסדרן המדויק.
--
-- הגדרת ה-view משוכפלת כאן במלואה מ-005, כולל ה-WHERE שמשכפל במפורש את כלל
-- הראייה במקום להסתמך על RLS - זו הסיבה שמותר לה לרוץ כ-owner (ר' ההסבר המלא
-- במיגרציה 005). אין כאן שינוי הרשאות, רק תוספת עמודה.
...
-- השלמת קוד לחשבונות שכבר הוזנו בהקלדה חופשית, לפי שם הבנק. resolve_bank_code
-- מחזיר null כשהשם אינו מזוהה, ואז העמודה נשארת ריקה - עדיף מאשר לנחש קוד,
-- שהיה שולח כסף לבנק אחר.
update organization_bank_accounts
set bank_code = resolve_bank_code(bank_name)
where bank_code is null
  and bank_name is not null
  and resolve_bank_code(bank_name) is not null;


-- 095 חישבה את קוד הבנק (resolve_bank_code) אך לא שמרה אותו - העמודה עוד לא
-- הייתה קיימת. עכשיו שהיא קיימת, הפונקציה נוצרת מחדש כדי לשמור אותו בפועל.
-- כל השאר זהה ל-095.
  ...
  for v_row in select * from import_rows where batch_id = p_batch_id and status = 'valid' order by row_number loop
    ...
    if v_account_number is not null then
      ...
      if not exists (
        ...
        insert into organization_bank_accounts (organization_id, bank_name, bank_code, bank_branch_code, account_number, is_active)
        values (v_org_id, v_bank_name, v_bank_code, v_bank_branch, v_account_number, true);
