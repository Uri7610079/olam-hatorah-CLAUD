-- Patch לפרויקט החי: commit_eligibility_batch()/commit_errors_batch() (018/019, כבר רצו)
-- ניסו לסמן import_batches.status='committed' בלי להפעיל קודם את הדגל
-- app.allow_batch_commit - הטריגר import_batches_enforce_commit (013) חסם אותן, בדיוק
-- כפי שהוא אמור לחסום כל נסיון ישיר אחר. תוקן ב-018/019 על הדיסק; זה ה-ALTER לפרויקט הקיים.
