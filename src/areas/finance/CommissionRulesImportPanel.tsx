...
export function CommissionRulesImportPanel({ orgId }: CommissionRulesImportPanelProps) {
  ...
  return (
    <div className="card max-w-3xl space-y-4 p-5">
      <p className="text-sm text-ink-muted">יבוא כללי עמלה בכמות מקובץ אקסל/CSV.</p>

      {!reviewBatchId && (
        <div className="space-y-4">
          <p className="text-xs text-ink-subtle">
            עמודות צפויות בקובץ: שם קבוצה (לא חובה), קוד לימוד (לא חובה), מזהה תלמיד חיצוני (לא חובה), סוג חישוב (אחוז/קבוע/משולב), אחוז, סכום קבוע, כלל עיגול (לא חובה: ללא/לקרוב/כלפי מעלה/כלפי מטה), צעד עיגול (לא חובה: 1, 5, 10…), עיגול על (לא חובה: עמלה/נטו), עדיפות (לא חובה), תקף מתאריך, תקף עד תאריך (לא חובה), הערות.
          </p>
      ...
      {commitResult && (
        <div className="rounded-md bg-ok-soft p-3 text-sm text-ok-ink">
          נוצרו <span className="tabular">{commitResult.created}</span> כללי עמלה.{" "}
      ...
      {!reviewBatchId && (
        <>
          <h3 className="mb-2 mt-2 text-sm font-semibold text-ink-muted">היסטוריית יבוא כללי עמלה</h3>
          <DataTable
            columns={[
