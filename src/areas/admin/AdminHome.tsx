      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
        {screens.map((s) => (
          <Link key={s.path} to={s.path} className="card flex items-start gap-3 px-4 py-3.5 transition hover:-translate-y-0.5 hover:shadow-md">
            ...
            <div>
              <p className="text-sm font-medium text-ink">{s.navLabel}</p>
              <span className="mt-1.5 inline-flex items-center rounded-full bg-surface-muted px-2 py-0.5 text-xs font-medium text-ink-subtle">
                {s.builtInStage}
              </span>
            </div>
          </Link>
