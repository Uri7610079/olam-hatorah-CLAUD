...
interface StudyCodesBatchSummary {
  id: string;
  file_name: string;
  status: BatchStatus;
  valid_count: number;
...
async function fetchStudyCodesBatches(): Promise<StudyCodesBatchSummary[]> {
  const profile = await supabase.from("import_profiles").select("id").eq("key", "study_codes").single();
...
async function fetchBatchById(id: string): Promise<StudyCodesBatchSummary> {
  const { data, error } = await supabase
    .from("import_batches")
    .select("id, file_name, status, valid_count, needs_decision_count, invalid_count, created_at")
    .eq("id", id)
...
// פאנל יבוא קודי לימוד מאקסל - אותו דפוס בדיוק כמו StudentsImportPanel/EligibilityImportPanel,
// בלי בורר עמותה/חודש (קוד לימוד הוא ערך מערכת גלובלי, לא קשור לעמותה).
export function StudyCodesImportPanel() {
  const queryClient = useQueryClient();
  const { hasPermission: canImport } = useHasPermission("study_codes", "manage");
  ...
  const batchesQuery = useQuery({ queryKey: ["study-codes-import-batches"], queryFn: fetchStudyCodesBatches });
  const reviewBatchQuery = useQuery({ queryKey: ["study-codes-import-batch", reviewBatchId], queryFn: () => fetchBatchById(reviewBatchId!), enabled: !!reviewBatchId });
  const reviewRowsQuery = useQuery({
    queryKey: ["study-codes-import-batch-rows", reviewBatchId],
  ...
  const closeReview = () => {
    ...
    queryClient.invalidateQueries({ queryKey: ["study-codes-import-batches"] });
  };
  ...
  const submitBatch = async () => {
    ...
    setError(null);
    try {
      const { batchId } = await createImportBatch({ file, profileKey: "study_codes" }, parsedRows);
      resetForm();
      queryClient.invalidateQueries({ queryKey: ["study-codes-import-batches"] });
  ...
  const resolveRow = async (rowNumber: number, status: "valid" | "invalid") => {
    ...
    queryClient.invalidateQueries({ queryKey: ["study-codes-import-batch-rows", reviewBatchId] });
  };
  ...
  const commit = async () => {
    ...
    setCommitting(true);
    setError(null);
    const { data, error: commitError } = await supabase.rpc("commit_study_codes_import_batch", { p_batch_id: reviewBatchId }).single();
    setCommitting(false);
    if (commitError) {
    ...
    queryClient.invalidateQueries({ queryKey: ["study-codes-import-batch", reviewBatchId] });
    queryClient.invalidateQueries({ queryKey: ["study-codes-import-batch-rows", reviewBatchId] });
    queryClient.invalidateQueries({ queryKey: ["study-codes-import-batches"] });
    queryClient.invalidateQueries({ queryKey: ["study-codes"] });
  };

  if (!canImport) return <ErrorState message="אין לך הרשאה ליבוא קודי לימוד." />;
        ...
        <DataTable
          columns={[
            { key: "file", header: "קובץ", render: (b: StudyCodesBatchSummary) => b.file_name },
            {
              ...
              className: "tabular",
              render: (b: StudyCodesBatchSummary) => `${b.valid_count} / ${b.needs_decision_count} / ${b.invalid_count}`,
            },
            {
              key: "status",
              header: "סטטוס",
              render: (b: StudyCodesBatchSummary) => (
                <StatusBadge severity={b.status === "committed" ? "ok" : b.status === "rejected" ? "neutral" : "medium"} label={BATCH_STATUS_LABEL[b.status]} />
              ),
            },
            { key: "date", header: "תאריך", className: "tabular", render: (b: StudyCodesBatchSummary) => new Date(b.created_at).toLocaleDateString("he-IL") },
          ]}
          rows={batchesQuery.data ?? []}
          rowKey={(b: StudyCodesBatchSummary) => b.id}
          loading={batchesQuery.isLoading}
          emptyTitle="אין עדיין יבואים"
          onRowClick={(b: StudyCodesBatchSummary) => setReviewBatchId(b.id)}
        />
      </div>
    </div>
  );
