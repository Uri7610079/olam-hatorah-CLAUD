...
export function TaskCommentsTab({ taskId, canEdit, userId }: TaskCommentsTabProps) {
  ...
  return (
    <div className="space-y-5">
      <div>
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-subtle">הערות</h3>
        <div className="space-y-2">
          {comments.length === 0 && <EmptyState title="אין הערות עדיין" />}
          {comments.map((c) => (
            <div key={c.id} className="rounded-control border border-line p-2.5">
              {editingId === c.id ? (
                <div className="space-y-2">
                  <textarea value={editingBody} onChange={(e) => setEditingBody(e.target.value)} rows={2} className="input-field" />
                  <div className="flex justify-end gap-2">
                    <button onClick={() => setEditingId(null)} className="text-xs text-ink-subtle underline">
                      ביטול
                    </button>
      ...
      <div>
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-ink-subtle">קבצים מצורפים</h3>
        {error && <ErrorState message={error} />}
        <div className="space-y-1.5">
          {attachments.length === 0 && <EmptyState title="אין קבצים מצורפים" icon={Paperclip} />}
          {attachments.map((a) => (
            <div key={a.id} className="flex items-center justify-between gap-2 rounded-control border border-line p-2 text-sm">
