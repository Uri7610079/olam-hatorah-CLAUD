...
interface AuditsContainerBatchSummary {
  id: string;
  file_name: string;
  status: BatchStatus;
  valid_count: number;
...
async function fetchBatches(orgId: string): Promise<AuditsContainerBatchSummary[]> {
  const profile = await supabase.from("import_profiles").select("id").eq("key", "audits_container").single();
...
async function fetchBatchById(id: string): Promise<AuditsContainerBatchSummary> {
  const { data, error } = await supabase
    .from("import_batches")
    .select("id, file_name, status, valid_count, needs_decision_count, invalid_count, created_at")
    .eq("id", id)
...
// פאנל יבוא קבוצתי של אירועי ביקורת עצמם (הקונטיינר - עמותה/סניף/תאריך), לא רשימת
// חסרים לתוך ביקורת קיימת (זה נשאר "יבוא רשימת חסרים" הקיים, כפתור נפרד ב-AuditsScreen).
// אותו דפוס בדיוק כמו EligibilityImportPanel (analyzeFile/createImportBatch/commit
// ייעודי-דומיין), פרופיל 'audits_container' נפרד לגמרי מ-'audit_attendance'.
export function AuditsContainerImportPanel() {
  const queryClient = useQueryClient();
  const { hasPermission: canManage, isLoading: permissionLoading } = useHasPermission("audits", "manage");

  const [orgId] = useLastSelected<string>("last-org", "");
  ...
  const batchesQuery = useQuery({ queryKey: ["audits-container-batches", orgId], queryFn: () => fetchBatches(orgId), enabled: !!orgId });
  const reviewBatchQuery = useQuery({ queryKey: ["audits-container-batch", reviewBatchId], queryFn: () => fetchBatchById(reviewBatchId!), enabled: !!reviewBatchId });
  const reviewRowsQuery = useQuery({
    queryKey: ["audits-container-batch-rows", reviewBatchId],
  ...
  const closeReview = () => {
    ...
    queryClient.invalidateQueries({ queryKey: ["audits-container-batches", orgId] });
  };
  ...
  const submitBatch = async () => {
    ...
    setError(null);
    try {
      const { batchId } = await createImportBatch({ file, profileKey: "audits_container", organizationId: orgId }, parsedRows);
      resetForm();
      queryClient.invalidateQueries({ queryKey: ["audits-container-batches", orgId] });
  ...
  const resolveRow = async (rowNumber: number, status: "valid" | "invalid") => {
    ...
    queryClient.invalidateQueries({ queryKey: ["audits-container-batch-rows", reviewBatchId] });
  };
  ...
  const commit = async () => {
    ...
    const { data, error: commitError } = await supabase
      .rpc("commit_audits_import_batch", { p_batch_id: reviewBatchId, p_organization_id: orgId })
      .single();
    setCommitting(false);
    if (commitError) {
    ...
    queryClient.invalidateQueries({ queryKey: ["audits-container-batch", reviewBatchId] });
    queryClient.invalidateQueries({ queryKey: ["audits-container-batch-rows", reviewBatchId] });
    queryClient.invalidateQueries({ queryKey: ["audits-container-batches", orgId] });
  ...
  if (!canManage) return <ErrorState message="אין לך הרשאה ליצור אירועי ביקורת." />;
  if (!orgId) return <ErrorState message="יש לבחור עמותה למעלה כדי לייבא אירועי ביקורת." />;
      ...
      <DataTable
        columns={[
          { key: "file", header: "קובץ", render: (b: AuditsContainerBatchSummary) => b.file_name },
          {
            ...
            className: "tabular",
            render: (b: AuditsContainerBatchSummary) => `${b.valid_count} / ${b.needs_decision_count} / ${b.invalid_count}`,
          },
          {
            key: "status",
            header: "סטטוס",
            render: (b: AuditsContainerBatchSummary) => (
              <StatusBadge severity={b.status === "committed" ? "ok" : b.status === "rejected" ? "neutral" : "medium"} label={BATCH_STATUS_LABEL[b.status]} />
            ),
          },
          { key: "date", header: "תאריך", className: "tabular", render: (b: AuditsContainerBatchSummary) => new Date(b.created_at).toLocaleDateString("he-IL") },
        ]}
        rows={batchesQuery.data ?? []}
        rowKey={(b: AuditsContainerBatchSummary) => b.id}
        loading={batchesQuery.isLoading}
        emptyTitle="אין עדיין יבואים"
        onRowClick={(b: AuditsContainerBatchSummary) => setReviewBatchId(b.id)}
      />
    </div>
  );
}
